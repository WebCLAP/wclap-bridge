...
function translate(use64) {
	...
	const translatorClass = `WclapTranslator<${use64 ? 'true' : 'false'}>`;
	const scopeClass = `WclapTranslationScope<${use64 ? 'true' : 'false'}>`;
	...
	*/
	...
	function addObjectType(name, contextField, destroyFn="destroy") {
		objectStructInfo[name] = {
			contextField: contextField,
			destroyFn: destroyFn
		};
		addCustomField(name, contextField, pointerType); // TODO: generate code which points this to a translation context
		addCustomField(name, destroyFn, pointerType); // TODO: map to function which destroys the proxy object (and translation context) after calling the WASM one
	}
	...
	function addDirect(type, wasmSize, wasmAlign, wasmType) {
		...
		if (objectStructInfo[type]) {
			structTypes[type].object = objectStructInfo[type];
		}
			...
			fields.forEach(cpp => {
				...
				if (match = cpp.match(/^(.*[^a-zA-Z_])([a-zA-Z_][a-zA-Z0-9_]*)(\[([0-9]+)\])?$/)) {
					...
					fArgs = fArgs.map((arg, argIndex) => {
						...
						let argType = structTypes[arg];
						if (!argType) {
							console.error(cpp);
							throw Error("Unknown argument type in struct " + name + ": " + arg);
						}
						if (argIndex == 0 && argType.pointerToType) {
							argType.pointerToType.calledAsObject = true;
						}
						return arg;
					});
					if (!fArgs.length) throw Error("Method has no arguments: " + name + ":" + fName);
	...
	console.error("Unknown return type in struct " + name + ": " + fReturn);
			...
			if (!structType.compatible) {
				...
				code += `
template<>
struct Wclap${bits}TranslateStruct<${name}> {
	using WasmP = uint${bits}_t;
	static void assignWasmToNative(${scopeClass} *translate, WasmP wasmP, ${nativeType} &native) {`;
...
${arrayIndent}		translate->assignWasmToNativeDirectPointer<${field.type}>(${wasmPointerArg}, native.${field.name}${arraySuffix});`;
						...
						code += `
${arrayIndent}		native.${field.name}${arraySuffix} = callWasmFromNative_${field.name};`;
...
${arrayIndent}		translate->assignWasmToNative<${field.type}>(${wasmPointerArg}, native.${field.name}${arraySuffix});`;
				...
				code += `
	static void assignNativeToWasm(${scopeClass} *translate, ${name} const &native, WasmP wasmP) {`;
				structFields.forEach(field => {
					let wasmPointerArg = field.offset ? "wasmP + " + field.offset : "wasmP";
					let arraySuffix = '', arrayIndent = '';
					if (field.arrayCount) {
...
${arrayIndent}		translate->assignNativeToWasmDirectPointer<${field.type}>(native.${field.name}${arraySuffix}, ${wasmPointerArg});`;
						...
						code += `
${arrayIndent}		*(WasmP *)${wasmPointerArg} = translate.wasmMethods.${name}.${field.name}; // ${field.returnType || 'void'} ${field.name}${arraySuffix}(${field.argTypes})`;
					} else if (customFields[name]?.[field.name]) {
						code += `
${arrayIndent}		translate->assignNativeToWasm_${name}_${field.name}(native.${field.name}${arraySuffix}, ${wasmPointerArg});`;
						...
						code += `
${arrayIndent}		translate->assignNativeToWasm<${field.type}>(native.${field.name}${arraySuffix}, ${wasmPointerArg});`;
					}
					if (field.arrayCount) {
						code += `
		}`;
				...
				if (structFields.some(field => field.argTypes)) {
					code += `
	...
	// method proxies`;
				...
				structFields.forEach(field => {
					if (field.argTypes) {
						code += `
	${field.returnType || 'void'} callWasmFromNative_${field.name}(${field.argTypes.map((type, index) => type + (type.endsWith('*') ? "arg" : " arg") + index).join(", ")}) {`
						let objArg = field.argTypes[0];
						let objType = objArg;
						let objInfo = structType[objType]?.pointerToType?.object;
						if (objInfo) {
							code += `
		${JSON.stringify(objInfo)}
		auto *context = (${scopeClass} *)arg0->${objInfo.contextField};`;
						} else {
							code += `
		beep boop
		auto *obj0 = (WclapLifetimeStruct<${objArg.replace(/\s*\*$/, '')}> *)arg0;
		auto *context = (${scopeClass} *)obj0->wclapTranslationScope;`;
						}
						code += `
	}`;
					}
	...
	addObjectType("clap_ostream_t", "ctx", null);
	...
	addObjectType("clap_input_events_t", "ctx", null);
	addObjectType("clap_output_events_t", "ctx", null);
	addCustomField("clap_plugin_descriptor_t", "features");
	addObjectType("clap_plugin_t", "plugin_data");
	addCustomField("clap_plugin_t", "get_extension");
	addObjectType("clap_preset_discovery_metadata_receiver_t", "receiver_data");
	addObjectType("clap_preset_discovery_provider_t", "provider_data");
	addCustomField("clap_preset_discovery_provider_t", "get_extension");
	addObjectType("clap_preset_discovery_indexer_t", "indexer_data");
	...
	addObjectType("clap_context_menu_builder_t", "ctx", null);
	...
	// We don't actually instantiate a plugin for now.  This is enough to inspect the list of plugins.
...
template<>
struct ${translatorClass} {`;
	for (let name in structTypes) {
		let structType = structTypes[name];
		if (!structType.compatible && /^[a-z0-9_]+$/g.test(name)) {
			code += `
	Wclap32TranslateStruct<${name}> ${name};`;
		}
	}
	code += `
	...
	fs.writeFileSync(__dirname + `/source/wclap${bits}-translate-struct.generated.h`, code);
}
