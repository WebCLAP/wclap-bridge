...
function translate(bits) {
	const wasmPointerType = `uint${bits}_t`;
	const wasmPointerSize = bits/8;
	const wasmPointerTypeCode = (bits == 32 ? 'i' : 'I');
...
struct Wclap${bits}TranslateStruct<${name}> {
	...
	static void assignWasmToNative(WclapTranslationScope *translate, WasmP wasmP, ${name} &native) {`;
...
${arrayIndent}		translate->assignWasmToNativeDirectPointer<${field.type}>(${wasmPointerArg}, native.${field.name}${arraySuffix});`;
						...
						code += `
${arrayIndent}		translate->assignWasmToNative_${field.wasmFnType}(${wasmPointerArg}, native.${field.name}${arraySuffix}); // ${field.returnType || 'void'} ${field.name}${arraySuffix}(${field.argTypes})`;
...
${arrayIndent}		translate->assignWasmToNative<${field.type}>(${wasmPointerArg}, native.${field.name}${arraySuffix});`;
	...
	// translate all the fields
	static void assignNativeToWasm(WclapTranslationScope *translate, ${name} const &native, WasmType wasmP) {`;
				structFields.forEach(field => {
					let wasmPointerArg = field.offset ? "wasmP + " + field.offset : "wasmP";
					let arraySuffix = '', arrayIndent = '';
					if (field.arrayCount) {
