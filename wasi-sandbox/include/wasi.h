 ...
 * \file wasi.h
 *
 * C API for WASI, adapted from Wasmtime
 */
 ...
 * - WASI_FILE_PERMS_READ
 * - WASI_FILE_PERMS_WRITE
 */
 ...
 *
 * \fn void wasi_instance_delete(wasi_instance_t *);
 ...
 */
WASI_API_EXTERN own wasi_instance_t *wasi_instance_new(const wasi_config_t *config, wasm_store_t *store);
 ...
 */
WASI_API_EXTERN bool wasi_instance_link_memory(wasi_instance_t *instance, wasm_memory_t *memory);
 ...
 */
WASI_API_EXTERN wasm_extern_t * wasi_instance_resolve(wasi_instance_t *instance, const wasm_importtype_t *type);
